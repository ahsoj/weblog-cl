...
router.post('/article/upload', async (req: Request, res: Response) => {
  try {
...
router.post('/article/create', async (req: Request, res: Response) => {
  try {
    ...
    if (!current_user) return res.status(401).send('Un-Authorized request.');
